//this does ordinary diffusion rather than ballistic (general BETA is now done properly )
...
int main()
{
  ...
  */
  ...
  densdata=fopen("denssimple2diffsigavN18khot8-1r-.125ML","w");
  dist1=fopen("isdsimple2diffsigavN18khot8-1r-.025ML","w");
  dist2=fopen("isdsimple2diffsigavN18khot8-1r-.05ML","w");
  dist3=fopen("isdsimple2diffsigavN18khot8-1r-.075ML","w");
  dist4=fopen("isdsimple2diffsigavN18khot8-1r-.1ML","w");
  dist5=fopen("isdsimple2diffsigavN18khot8-1r-.125ML","w");
  picdata=fopen("picsimple2diffsigavN18khot8-1r-.125ML","w");
...
void takecapavdata (int ilogdata, double countmin) {
  double numdiff=0;
    ...
    double count1;
    int ii,i;
    //first save dira,list, and lenwalka 
    for(i=0;i<nw[0];i++){
      //      dira2[i]=dira[i];
  ...
  //  printf("we are in diffuse i=%d j=%d h=%d -> in=%d jn=%d h=%d \n",x,y,h[x][y],xnb,ynb,h[xnb][ynb]);
    ...
    int iplace;
    h[xnb*N+ynb]--;//remove walker since it has neighbors
    ...
    if(iwalk2!=-1){//if nbr is monomer
      h[inb*N+jnb]--;//removethe neighboring monomer as well
