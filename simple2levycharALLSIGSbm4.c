...
//int *islsize;
#ifdef RESTORE
int dira2[MAXNUMMONOMERS],list2[MAXNUMMONOMERS],lenwalka2[MAXNUMMONOMERS];
unsigned char *h2;
#endif
//int imark[N][N];
int *imark;
...
int *ipointa;
//int h[N][N],imark[N][N];
int dira[MAXNUMMONOMERS],list[MAXNUMMONOMERS],lenwalka[MAXNUMMONOMERS];
...
double ct[MAXISLSIZE];
int islandsize[MAXNUMCLUSTERS];
// main parameters
double pi,diffq;
double pbarr=1.0;
/* run parameters */
...
void takecapavdata(int idata,double x);//number of attachments per island
//int checknbonds(int xnb,int ynb);//checkclearsite returns direction of first higher site found if any, otherwise it returns -1
...
int main()
{
...
#ifdef RESTORE
  h2 = (unsigned char *)malloc(sizeof(unsigned char)*(N*N));
#endif
  ...
  capdist1=fopen("capdistbeta1sigsc100-N18khot7-1r-.025ML-OSC","w");
  capdist2=fopen("capdistbeta1sigsc100-N18khot7-1r-.05ML-OSC","w");
  capdist3=fopen("capdistbeta1sigsc100-N18khot7-1r-.075ML-OSC","w");
  capdist4=fopen("capdistbeta1sigsc100-N18khot7-1r-.1ML-OSC","w");
  capdist5=fopen("capdistbeta1sigsc100-N18khot7-1r-.125ML-OSC","w");

  
  //  capdata=fopen("capavsimple2levycharsigbeta1sigsN5khot8-1r-.125ML-OSC","w");
  densdata=fopen("densbeta1sigsc100-N18khot7-1r-.125ML-OSC","w");
  dist1=fopen("isdbeta1sigsc100-N18khot7-1r-.025ML-OSC","w");
  dist2=fopen("isdbeta1sigsc100-N18khot7-1r-.05ML-OSC","w");
  dist3=fopen("isdbeta1sigsc100-N18khot7-1r-.075ML-OSC","w");
  dist4=fopen("isdbeta1sigsc100-N18khot7-1r-.1ML-OSC","w");
  dist5=fopen("isdbeta1sigsc100-N18khot7-1r-.125ML-OSC","w");
  picdata=fopen("picbeta1sigsc100-N18khot7-1r-.125ML-OSC","w");
  ...
  */
  ...
  for(irun=0;irun < NRUN;irun++){
	...
	if(cov >= covnext){ 
	  ...
	  printf("ideposit=%d idiffuse=%d nw[0]=%d nw[1]=%d nw[2]=%d nw[3]=%d\n",ideposit,idiffuse,nw[0],nw[1],nw[2],nw[3]);
	  covnext=covnext*covfactor;
	  //let's also take average capture number data
	  ilogdata++;
	  printf("ilogdata=%d\n",ilogdata);
#ifdef DEBUG
	  //now go through lattice and check to see if all ipointa correspond to list
	  for(i=0;i<N;i++){
	    for(j=0;j<N;j++){
	      iwalk=ipointa[i*N+j];
	      if(iwalk!=-1){
		ii=list[iwalk]/N;jj=list[iwalk]%N;
		if(ii!=i || jj!=j){printf("list and ipointa don't match!\n");abort();}
	      }
	    }}   //now go through lattice and check to see if all monomers on list have size 1
	  int iabort=0;
	  int icc,isite;
	      for(iwalk=0;iwalk<nw[0];iwalk++){
		isite=list[iwalk];
		ii=list[iwalk]/N;jj=list[iwalk]%N;
		if(islandsize[imark[isite]]!=1 && h[isite]=='1'){icc=icount(ii,jj);
		  printf("we found substrate monomer on list with island-size %d iwalk=%d  ii=%d jj=%d icount=%d imark=%d ht=%c\n",islandsize[imark[list[iwalk]]],iwalk,ii,jj,icc,imark[isite],h[isite]);
iabort=1;}
	      }
	      if(iabort==1)abort();
	      //now go though lattice and make sure all sites with ipointa=-1 and h>0 have island-size >1
	  for(i=0;i<N;i++){
	    for(j=0;j<N;j++){
	      iwalk=ipointa[i*N+j];
	      if(iwalk==-1 && h[i*N+j] > '0'){
		if(islandsize[imark[i*N+j]]==1){printf("we found non-monomer site with size 1 h=%c\n",h[i*N+j]);abort();}
	      }
	    }}
#endif 
	}
	...
	if (cov >= (idata+2)*dcov){ 
	  ...
	  for(i=0;i<N;i++){
	    for(j=0;j<N;j++){
	      ...
	      if(iwalk!=-1){
		ii=list[iwalk]/N;jj=list[iwalk]%N;
		if(ii!=i || jj!=j){printf("before dist list and ipointa don't match!\n");abort();}
	      }
	    }}   //now go through lattice and check to see if all monomers on list have size 1
	  int iabort=0;
	  int icc,isite;
		...
		ii=list[iwalk]/N;jj=list[iwalk]%N;
		if(islandsize[imark[isite]]!=1 && h[isite]=='1'){icc=icount(ii,jj);
		  printf("before dist we found substrate monomer on list with island-size %d iwalk=%d  ii=%d jj=%d icount=%d imark=%d ht=%c\n",islandsize[imark[list[iwalk]]],iwalk,ii,jj,icc,imark[isite],h[isite]);
iabort=1;}
	      }
	      if(iabort==1)abort();
	      //now go though lattice and make sure all sites with ipointa=-1 and h>0 have island-size >1
	  for(i=0;i<N;i++){
	    for(j=0;j<N;j++){
	      iwalk=ipointa[i*N+j];
	      if(iwalk==-1 && h[i*N+j] > '0'){
		if(islandsize[imark[i*N+j]]==1){printf("before dist we found island-site with size 1\n");abort();}
	      }
	    }}
#endif
...
void takelogdata (int ilogdata)
{
  ...
  for(i=0;i<MAXNUMCLUSTERS;i++) {
    islandsize[i]=0;}/* I added(jga) this here */
  ...
  for(i=0;i<N;i++){
      ...
      if(h[i*N+j]> '0' && imark[i*N+j]== 0) 
	{
	  ...
	  if(numcl>MAXNUMCLUSTERS){printf("numcl=%d MAXNUMCLUSTERS=%d\n",numcl,MAXNUMCLUSTERS);abort();}
	  check(i,j,numcl);// this is recursive call
	     	islandsize[numcl]=isize;
  ...
  for(i=0;i<N;i++){
    for(j=0;j<N;j++){
 	...
 	if(h[i*N+j]>0){
	numcl=imark[i*N+j];
	//	islsizea[i*N+j]=islsiza[numcl];
	}//endif
	else islsizea[i*N+j]=-1;
	//    }
...
void takedistdata()
{
  ...
  for(i=0;i<N;i++){
	...
	check(i,j,numcl);
	islandsize[numcl]=isize;
	if(isize >1)numisland++;
	islcnta[idata][isize]=islcnta[idata][isize]+1;
	numisizea[isize]=numisizea[isize]+1;
	if(isize > isizemax)isizemax=isize;
      }
    }}/* i,j loop over system */

  /*
  for(i=0;i<N;i++){//2nd pass to mark island-sites  with size
    for(j=0;j<N;j++){
      //      for(k=0;k<N;k++){
 	if(h[i][j]>0){
	numcl=imark[i][j];
...
/*
 int checknbonds(int xnb,int ynb){//checknbrs returns direction of first bond-site found if any, otherwise it returns -1
   int in,jn,dir;
...
int icount(int i, int j) // this determines class of particle 
{
  ...
  int ibond,ichk,ibonda[6],nchk,nchk1,nchk2;
  int bdira[10];
  unsigned char hx;
